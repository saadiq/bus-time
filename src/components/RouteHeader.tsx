    ...
    <header className="card px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          {originName && destinationName ? (
            <div className="flex items-center gap-2 min-w-0 text-sm">
              <span className="truncate text-(--text-primary) font-medium">{originName}</span>
              <button
                onClick={onSwapDirections}
                className="shrink-0 p-1 rounded-md hover:bg-(--bg-card) transition-colors"
              ...
              <span className="truncate text-(--text-primary) font-medium">{destinationName}</span>
            </div>
          ) : (
        ...
        <button
          ...
          className="shrink-0 p-2 rounded-lg hover:bg-(--bg-card) transition-colors"
          aria-label="Settings"
        >
      ...
      {enableCutoff && cutoffTime && (
        <div className="mt-1.5 text-xs text-(--text-secondary) font-medium pl-1">
